...
def gconnect():
    ...
    if result['user_id'] != gplus_id:
        ...
        user ID doesn't match given user ID."), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(json.dumps("Token's \
        client ID does not match app's."), 401)
...
def editRestaurant(restaurant_id):
    ...
    if editedRestaurant.user_id != login_session['user_id']:
       ...
       this restaurant. please create your\
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('you are not authorized to \
         delete this restaurant.please create\
         your own restaurant to delete');}\
         </script><body onLoad = 'myFunction()''>"
    if request.method == 'POST':
